...
describe("SERVICE: serviceDeleteComment (user)", () => {
  beforeEach(() => {
    mockReturnValues.length = 0;
    jest.clearAllMocks();
  });

  it("SUCCESS: should delete comment when owner", async () => {
    mockReturnValues.push([{ id: 1, user_id: 1 }]);
    mockReturnValues.push([]);

    const result = await serviceDeleteComment(1, 1);
  ...
  it("ERROR: should throw 404 when comment does not exist", async () => {
    ...
    await expect(serviceDeleteComment(999, 1))
      .rejects
  ...
  it("ERROR: should throw 403 when user is not the owner", async () => {
    mockReturnValues.push([{ id: 1, user_id: 2 }]);

    await expect(serviceDeleteComment(1, 1))
      .rejects
      .toThrow(new CustomError("Anda tidak memiliki akses untuk menghapus komentar ini", 403));
